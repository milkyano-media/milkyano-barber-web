...
export const OTPVerificationModal = ({
  ...
  const [modalType, setModalType] = useState<OTPModalType>(
    OTPModalType.VERIFICATION
  );
  ...
  // Reset modal type when modal is opened
  useEffect(() => {
    if (isOpen) {
      setModalType(OTPModalType.VERIFICATION);
    }
  }, [isOpen]);
  ...
  const onSubmit = async (data: OTPFormData) => {
    try {
      ...
      // Switch to success modal instead of toast
      setModalType(OTPModalType.SUCCESS);
      form.reset();
  ...
  const handleSuccessClose = () => {
    ...
    onSuccess?.();
    // onClose();
    // Reset modal type for next use
    setModalType(OTPModalType.VERIFICATION);
  };
  const renderModalContent = () => {
    switch (modalType) {
      case OTPModalType.SUCCESS:
        return (
          <>
      ...
      case OTPModalType.VERIFICATION:
      default:
        return (
          <>
            <DialogHeader className="space-y-4">
